...
struct shadowPlusColor {
    ...
    v_size randChoose(v_size c, std::default_random_engine & e) {
        std::uniform_int_distribution<v_size> d(1, a[c]);
        return pEdge[pIdx[c] + d(e)-1];
        // v_size deg = a[c];
    ...
    double sample(std::vector<v_size> & nodes) {
        ...
        while(sampleTotalTimes < sampleTimes) {
            ...
            int ret = sampleOneTime(uiDistribution, generator);
...
printf("not expected %llu\n", sampleTimes - sampleTotalTimes);
              while(sampleTotalTimes < sampleTimes) {
                  ...
                  t += sampleOneTime(uiDistribution, generator);
                  sampleTotalTimes++;
              }
