#ifndef TRIPARALLEL_HPP
#define TRIPARALLEL_HPP
...
#include "./sample/tripathParallel.h"
...
public:
    triParallel(Graph * g_, v_size threads_ = 1)
...
void triParallel::run(v_size k_, v_size deb, double exCnt, 
double alpha, e_size N) {
    // printf("begin run, dengenerarcy %u, vCnt %u\n", g->degeneracy, g->vCnt);
    k = k_;
    printf("k %u\nalpha %.1f\nsample size %lu\nthreads %u", k, alpha, N, threads); fflush(stdout);
    ...
    printf("exact tiem: %.2f\n", t1 - tS);fflush(stdout);
    
    printf("sparse part size: %u\n", sz);
    ...
    if(sz > 0) {//ccpathParallel
        ...
        tripathParallel * stpObj = new tripathParallel();

        stpObj->initForSingleNode(k-1, g, hashTable, threads);

        stpObj->init(sz, nodes, N);
...
void triParallel::computeDegeneracy(v_size threadId, v_size &mV) {
    for(v_size i = 0; i < subG[threadId]->vCnt; i++) {
        ids[threadId][i] = i;
...
void triParallel::search(v_size hNum, v_size pNum, Pair section, LinearSet * S, v_size threadId) {
    if(hNum > k) return;
