#ifndef CCPARALLEL_HPP
#define CCPARALLEL_HPP
...
public:
    ccParallel(Graph * g_, v_size threads_ = 1)
...
void ccParallel::run(v_size k_, v_size deb, double exCnt, 
double alpha, e_size N) {
    // printf("begin run, dengenerarcy %u, vCnt %u\n", g->degeneracy, g->vCnt);
    k = k_;
    printf("cc||%u| %.1f| %lu| %u", k, alpha, N, threads); fflush(stdout);
    ...
    if(sz > 0) {//ccpathParallel
        ...
        shadowPlusColorParallel * scpObj = new shadowPlusColorParallel();

        scpObj->initForSingleNode(k-1, g, hashTable, threads, sz);
...
void ccParallel::computeDegeneracy(v_size threadId, v_size &mV) {
    for(v_size i = 0; i < subG[threadId]->vCnt; i++) {
        ids[threadId][i] = i;
...
void ccParallel::search(v_size hNum, v_size pNum, Pair section, LinearSet * S, v_size threadId) {
    if(hNum > k) return;
