...
#include "../kclique/triParallel.hpp"
...
int main(int argc, char * argv[])
{
    ...
    if(deb == 0) {
        ...
        vector<double> allExcnt;
        if(~fscanf(f, "%u", &maxK)) {
            allExcnt.resize(maxK + 1);
            if(maxK >= k) {
...
if(aC->exist("-density")) {
    v_size stK = std::stoi(aC->get("-stk"));
    pt->getAllDensity(stK, k, allExcnt);
    delete g;
    delete pt;
    return 0;
}
        ...
        if(aC->exist("-cc")) {
            if(aC->exist("-auto")) {
                double stopBound = 0.05;
                if(aC->exist("-stop")) stopBound = std::stof(aC->get("-stop"));
                v_size stopHittedCnt = 3.0 * 4.60 / stopBound / stopBound;
                pt->runAutoCC(k, deb, exCnt, alpha, stopHittedCnt, N);
            }
            else
            ...
            if(aC->exist("-auto")) {
                double stopBound = 0.05;
                if(aC->exist("-stop")) stopBound = std::stof(aC->get("-stop"));
                v_size stopHittedCnt = 3 * 4.60 / stopBound / stopBound;
                pt->runAutoCCPath(k, deb, exCnt, alpha, stopHittedCnt, 10000);
            }
            else
            ...
            if(aC->exist("-auto")) {
                ...
                if(aC->exist("-stop")) stopBound = std::stof(aC->get("-stop"));
                v_size stopHittedCnt = 3 * 4.60 / stopBound / stopBound;
                pt->runAutoTriPath(k, deb, exCnt, alpha, stopHittedCnt, 10000);
            }
            else
        ...
        else if(aC->exist("-tri")) {
            triParallel * pt = 
                new triParallel(g, threads);
            pt->run(k, deb, exCnt, alpha, N);
            delete pt;
        }
        else {
